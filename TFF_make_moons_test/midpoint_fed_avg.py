...
@tf.function
def server_update_mid_point(server_weights, mean_client_weights):
    """Update the server model weights."""
    # Assign the mean client weights to the server model.
    _sub = tf.nest.map_structure(lambda x, y: x+y,
                          server_weights, mean_client_weights)
    half_sub =  tf.nest.map_structure(lambda x: x/2, _sub)
    return half_sub
#%% algorithm functions
...
def build_fed_avg_process(weights) -> tff.templates.IterativeProcess:
    ...
    @tff.tf_computation(server_state_type, server_state_type)
    def server_update_fn(server_state, mean_client_weights):
        """Perform the federated computation of the server update."""
        # return the updated server state
        return server_update_mid_point(server_state, mean_client_weights)
